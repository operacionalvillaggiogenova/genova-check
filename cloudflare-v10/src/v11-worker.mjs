...
async function initializeDatabase(env) {
  ...
  const t=now();
  for(const team of await rows(env.DB,"SELECT code FROM workflow_teams WHERE active=1")) teams.add(team.code);
...
async function activityView(env,a) {
  ...
  return {id:a.id,title:a.title,description:a.description,team:a.team,assignedToId:a.assigned_to_id,assignedTo:assigned,priority:a.priority,location:a.location,dueDate:a.due_date,status:a.status,type:a.type||"one_off",templateId:a.template_id||null,requiresObservation:!!a.requires_observation,createdById:a.created_by_id,createdBy:creator,startedAt:a.started_at,completedAt:a.completed_at,completedById:a.completed_by_id,source:a.source,createdAt:a.created_at,updatedAt:a.updated_at,overdue:!!a.due_date&&a.due_date<now()&&!["completed","cancelled"].includes(a.status)};
}
...
async function generateOccurrences(env, cutoff=now()) {
  const templates=await rows(env.DB,"SELECT * FROM activity_templates WHERE active=1 AND awaiting_activity_id IS NULL AND next_run_at<=? AND (ends_at IS NULL OR next_run_at<=ends_at) ORDER BY next_run_at LIMIT 100",cutoff);
  for(const t of templates) for(let count=0;t.next_run_at<=cutoff&&count<100;count++) { const scheduled=t.next_run_at, activityId=id(), occurrenceId=id(), created=now();
    await env.DB.batch([
      env.DB.prepare("INSERT OR IGNORE INTO activities(id,title,description,team,assigned_to_id,priority,location,due_date,status,type,template_id,created_by_id,source,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?, 'pending','recurring',?,?,?,?,?)").bind(activityId,t.title,t.description,t.team,t.assigned_to_id,t.priority,t.location,scheduled,t.id,t.created_by_id,"system",created,created),
    ...
    const templateLists=await rows(env.DB,"SELECT * FROM activity_template_checklists WHERE template_id=? ORDER BY created_at",t.id);
    for(const list of templateLists){const listId=id(),items=await rows(env.DB,"SELECT * FROM activity_template_checklist_items WHERE checklist_id=? ORDER BY position",list.id),operations=[env.DB.prepare("INSERT INTO activity_checklists(id,activity_id,title,created_by_id,created_at) VALUES(?,?,?,?,?)").bind(listId,activityId,list.title,t.created_by_id,created)];for(const item of items)operations.push(env.DB.prepare("INSERT INTO activity_checklist_items(id,checklist_id,label,position) VALUES(?,?,?,?)").bind(id(),listId,item.label,item.position));await env.DB.batch(operations);}
    await env.DB.prepare("UPDATE activity_templates SET awaiting_activity_id=?,updated_at=? WHERE id=? AND awaiting_activity_id IS NULL AND next_run_at=?").bind(activityId,created,t.id,scheduled).run(); break;
  }
}
...
async function v11(request, env, url, auth) {
  ...
  const convertRequest=path.match(/^\/api\/requests\/([^/]+)\/convert$/);if(convertRequest&&method==="POST"){
    const denied=require(auth,"activities.create");if(denied)return denied;const r=await one(env.DB,"SELECT * FROM requests WHERE id=?",convertRequest[1]);if(!r)return json({error:"Solicitação não encontrada"},404);if(r.status!=="pending")return json({error:"Solicitação já foi tratada"},409);const b=await body(request)||{},t=now(),activityId=id(),priority=priorities.has(b.priority)?b.priority:"normal",due=clean(b.dueDate)&&!Number.isNaN(Date.parse(b.dueDate))?new Date(b.dueDate).toISOString():new Date(Date.now()+86400000).toISOString();await env.DB.batch([env.DB.prepare("INSERT INTO activities(id,title,description,team,priority,location,due_date,status,type,created_by_id,source,created_at,updated_at) VALUES(?,?,?,?,?,?,?,'pending','one_off',?,'public_request',?,?)").bind(activityId,`Solicitação: ${r.category}`,r.description,r.team,priority,r.location,due,auth.user.id,t,t),env.DB.prepare("UPDATE requests SET status='converted',activity_id=?,converted_at=?,converted_by_id=? WHERE id=? AND status='pending'").bind(activityId,t,auth.user.id,r.id)]);await history(env,activityId,auth.user.id,"created",`Gerada da solicitação ${r.id}`);return json(await activityView(env,await getActivity(env,activityId)),201);
  }
  const notesMatch=path.match(/^\/api\/activities\/([^/]+)\/notes$/);if(notesMatch){const a=await getActivity(env,notesMatch[1]);if(!a||!visible(auth,a))return json({error:"Atividade não encontrada"},404);if(method==="GET"){const records=await rows(env.DB,"SELECT n.*,u.name author_name FROM activity_notes n JOIN users u ON u.id=n.created_by_id WHERE n.activity_id=? ORDER BY n.created_at DESC",a.id);return json(records.map(n=>({id:n.id,body:n.body,createdAt:n.created_at,author:{id:n.created_by_id,name:n.author_name}})));}if(method==="POST"){if(!coordinates(auth)&&!allowed(auth,"activities.start"))return json({error:"Permissão insuficiente"},403);const b=await body(request),text=clean(b?.body);if(text.length<1)return json({error:"Informe a observação"},400);const created=now(),note={id:id(),activity_id:a.id,body:text,created_by_id:auth.user.id,created_at:created};await env.DB.prepare("INSERT INTO activity_notes(id,activity_id,body,created_by_id,created_at) VALUES(?,?,?,?,?)").bind(note.id,note.activity_id,note.body,note.created_by_id,note.created_at).run();await history(env,a.id,auth.user.id,"note_added",text);return json({id:note.id,body:note.body,createdAt:created,author:{id:auth.user.id,name:auth.user.name}},201);}}
  const materialsMatch=path.match(/^\/api\/activities\/([^/]+)\/materials$/);if(materialsMatch){const a=await getActivity(env,materialsMatch[1]);if(!a||!visible(auth,a))return json({error:"Atividade não encontrada"},404);if(method==="GET")return json((await rows(env.DB,"SELECT m.*,u.name author_name FROM activity_materials m JOIN users u ON u.id=m.created_by_id WHERE m.activity_id=? ORDER BY m.created_at DESC",a.id)).map(m=>({id:m.id,name:m.name,quantity:m.quantity,unit:m.unit,createdAt:m.created_at,author:{id:m.created_by_id,name:m.author_name}})));if(method==="POST"){if(!coordinates(auth)&&!allowed(auth,"activities.start"))return json({error:"Permissão insuficiente"},403);const b=await body(request),name=clean(b?.name);if(name.length<1)return json({error:"Informe o material"},400);const created=now(),material={id:id(),activity_id:a.id,name,quantity:clean(b?.quantity)||null,unit:clean(b?.unit)||null,created_by_id:auth.user.id,created_at:created};await env.DB.prepare("INSERT INTO activity_materials(id,activity_id,name,quantity,unit,created_by_id,created_at) VALUES(?,?,?,?,?,?,?)").bind(material.id,material.activity_id,material.name,material.quantity,material.unit,material.created_by_id,material.created_at).run();await history(env,a.id,auth.user.id,"material_added",`${name}${material.quantity?` (${material.quantity}${material.unit?` ${material.unit}`:""})`:""}`);return json(material,201);}}
  ...
  if (evidenceMatch) {
    ...
    if (method==="POST") {
      ...
      const captured=clean(form.get("capturedAt")); const capturedAt=captured&&!Number.isNaN(Date.parse(captured))?new Date(captured).toISOString():now(), source=clean(form.get("source"))==="camera"?"camera":"gallery";
      const evidence={id:id(),activity_id:activity.id,r2_key:`activities/${activity.id}/evidence/${id()}-${file.name.replace(/[^a-zA-Z0-9._-]/g,"-")}`,filename:file.name,content_type:file.type||null,size:file.size,note:clean(form.get("note"))||null,uploaded_by_id:auth.user.id,created_at:now(),captured_at:capturedAt,source};
      await env.BUCKET.put(evidence.r2_key,file.stream(),{httpMetadata:{contentType:file.type||"application/octet-stream"}});
      await env.DB.prepare("INSERT INTO activity_evidence(id,activity_id,r2_key,filename,content_type,size,note,uploaded_by_id,created_at,captured_at,source) VALUES(?,?,?,?,?,?,?,?,?,?,?)").bind(evidence.id,evidence.activity_id,evidence.r2_key,evidence.filename,evidence.content_type,evidence.size,evidence.note,evidence.uploaded_by_id,evidence.created_at,evidence.captured_at,evidence.source).run();
  ...
  if(path==="/api/dashboard/summary"&&method==="GET"){const e=require(auth,"dashboard.view");if(e)return e;const a=await rows(env.DB,auth.role.name==="admin"?"SELECT * FROM activities ORDER BY CASE status WHEN 'pending' THEN 0 WHEN 'in_progress' THEN 1 ELSE 2 END, due_date,created_at DESC":"SELECT * FROM activities WHERE team=? OR assigned_to_id=? ORDER BY CASE status WHEN 'pending' THEN 0 WHEN 'in_progress' THEN 1 ELSE 2 END, due_date,created_at DESC",auth.user.team,auth.user.id);const active=x=>!["completed","cancelled"].includes(x.status),openActivities=a.filter(active),overdue=openActivities.filter(x=>x.due_date&&x.due_date<now()).length;return json({metrics:[["pending","Pendentes",a.filter(x=>x.status==="pending").length,"warning"],["in_progress","Em andamento",a.filter(x=>x.status==="in_progress").length,"info"],["overdue","Atrasadas",overdue,"danger"],["completed","Concluídas",a.filter(x=>x.status==="completed").length,"success"]].map(([key,label,value,tone])=>({key,label,value,tone})),recentActivities:await Promise.all(openActivities.map(x=>activityView(env,x)))});}
  ...
  if(path==="/api/activity-templates"&&method==="POST"){const e=require(auth,"activities.create");if(e)return e;const b=await body(request),team=clean(b?.team),starts=clean(b?.startsAt),kind=clean(b?.recurrenceType),interval=Math.max(1,Number.parseInt(b?.intervalValue,10)||1);if(clean(b?.title).length<3||!teams.has(team)||!priorities.has(b?.priority)||!["daily","weekly","monthly","custom"].includes(kind)||Number.isNaN(Date.parse(starts)))return json({error:"Dados da recorrência inválidos"},400);const assigned=clean(b?.assignedToId)||null,assignmentError=await assigneeError(env,team,assigned);if(assignmentError)return json({error:assignmentError},400);const ends=clean(b?.endsAt)||null;if(ends&&Number.isNaN(Date.parse(ends)))return json({error:"Data final inválida"},400);const t=now(),template={id:id(),title:clean(b.title),description:clean(b.description)||null,team,assigned,priority:b.priority,location:clean(b.location)||null,kind,interval,starts:new Date(starts).toISOString(),ends:ends?new Date(ends).toISOString():null};await env.DB.prepare("INSERT INTO activity_templates(id,title,description,team,assigned_to_id,priority,location,recurrence_type,interval_value,starts_at,ends_at,next_run_at,created_by_id,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(template.id,template.title,template.description,team,assigned,template.priority,template.location,kind,interval,template.starts,template.ends,template.starts,auth.user.id,t,t).run();for(const list of Array.isArray(b?.checklists)?b.checklists:[]){const title=clean(list?.title)||"Checklist",items=Array.isArray(list?.items)?list.items.map(clean).filter(Boolean):[];if(!items.length)continue;const listId=id(),operations=[env.DB.prepare("INSERT INTO activity_template_checklists(id,template_id,title,created_by_id,created_at) VALUES(?,?,?,?,?)").bind(listId,template.id,title,auth.user.id,t)];items.forEach((label,position)=>operations.push(env.DB.prepare("INSERT INTO activity_template_checklist_items(id,checklist_id,label,position) VALUES(?,?,?,?)").bind(id(),listId,label,position)));await env.DB.batch(operations);}await audit(env,auth.user.id,"activity_template.created","activity_template",template.id);return json(template,201);}
  const templateMatch=path.match(/^\/api\/activity-templates\/([^/]+)$/);if(templateMatch&&method==="PATCH"){const e=require(auth,"activities.update");if(e)return e;const b=await body(request),existing=await one(env.DB,"SELECT * FROM activity_templates WHERE id=?",templateMatch[1]);if(!existing||!visible(auth,existing))return json({error:"Recorrência não encontrada"},404);const team=clean(b?.team)||existing.team,kind=clean(b?.recurrenceType)||existing.recurrence_type,interval=Math.max(1,Number.parseInt(b?.intervalValue,10)||existing.interval_value),starts=clean(b?.startsAt),nextRunAt=starts?new Date(starts).toISOString():existing.next_run_at,ends=clean(b?.endsAt);if(clean(b?.title||existing.title).length<3||!teams.has(team)||!priorities.has(clean(b?.priority)||existing.priority)||!["daily","weekly","monthly","custom"].includes(kind)||Number.isNaN(Date.parse(nextRunAt))||(ends&&Number.isNaN(Date.parse(ends))))return json({error:"Dados da recorrência inválidos"},400);const assigned=b&&Object.hasOwn(b,"assignedToId")?(clean(b.assignedToId)||null):existing.assigned_to_id,assignmentError=await assigneeError(env,team,assigned);if(assignmentError)return json({error:assignmentError},400);await env.DB.prepare("UPDATE activity_templates SET title=?,description=?,team=?,assigned_to_id=?,priority=?,location=?,recurrence_type=?,interval_value=?,starts_at=?,ends_at=?,next_run_at=?,updated_at=? WHERE id=?").bind(clean(b?.title)||existing.title,clean(b?.description)||existing.description,team,assigned,clean(b?.priority)||existing.priority,clean(b?.location)||existing.location,kind,interval,starts?nextRunAt:existing.starts_at,ends||null,nextRunAt,now(),existing.id).run();await audit(env,auth.user.id,"activity_template.updated","activity_template",existing.id);return json(await one(env.DB,"SELECT * FROM activity_templates WHERE id=?",existing.id));}
  const templateHistory=path.match(/^\/api\/activity-templates\/([^/]+)\/history$/);if(templateHistory&&method==="GET"){const e=require(auth,"activities.view");if(e)return e;const t=await one(env.DB,"SELECT * FROM activity_templates WHERE id=?",templateHistory[1]);if(!t||!visible(auth,t))return json({error:"Recorrência não encontrada"},404);return json(await rows(env.DB,"SELECT o.scheduled_for,a.id,a.status,a.completed_at,a.title FROM activity_occurrences o JOIN activities a ON a.id=o.activity_id WHERE o.template_id=? ORDER BY o.scheduled_for DESC",t.id));}
  ...
  if(path==="/api/teams"&&method==="GET"){const e=require(auth,"activities.view");if(e)return e;return json(await rows(env.DB,"SELECT code,label,active,created_at,updated_at FROM workflow_teams ORDER BY active DESC,label"));}
  if(path==="/api/teams"&&method==="POST"){const e=require(auth,"users.manage");if(e)return e;const b=await body(request),label=clean(b?.label),code=clean(b?.code).toUpperCase().replace(/[^A-Z0-9_]/g,"_").replace(/^_+|_+$/g,"");if(label.length<3||code.length<3)return json({error:"Informe um nome de equipe válido"},400);const t=now();try{await env.DB.prepare("INSERT INTO workflow_teams(code,label,active,created_at,updated_at) VALUES(?,?,1,?,?)").bind(code,label,t,t).run();teams.add(code);await audit(env,auth.user.id,"workflow_team.created","workflow_team",code,{label});return json({code,label,active:true,createdAt:t},201);}catch{return json({error:"Já existe uma equipe com este nome ou código"},409)}}
  const teamMatch=path.match(/^\/api\/teams\/([^/]+)$/);if(teamMatch&&method==="PATCH"){const e=require(auth,"users.manage");if(e)return e;const existing=await one(env.DB,"SELECT * FROM workflow_teams WHERE code=?",teamMatch[1]);if(!existing)return json({error:"Equipe não encontrada"},404);const b=await body(request),label=clean(b?.label)||existing.label,active=Object.hasOwn(b||{},"active")?bool(b.active):!!existing.active;if(label.length<3)return json({error:"Nome de equipe inválido"},400);try{await env.DB.prepare("UPDATE workflow_teams SET label=?,active=?,updated_at=? WHERE code=?").bind(label,active?1:0,now(),existing.code).run();if(active)teams.add(existing.code);else teams.delete(existing.code);await audit(env,auth.user.id,"workflow_team.updated","workflow_team",existing.code,{label,active});return json({code:existing.code,label,active});}catch{return json({error:"Já existe uma equipe com este nome"},409)}}
  ...
  if(path==="/api/activities"&&method==="GET"){
    ...
    if(status){if(status==="open")q+=" AND status IN ('pending','in_progress')";else {if(!statuses.has(status))return json({error:"Status inválido"},400);q+=" AND status=?";args.push(status)}}
  ...
  if(path==="/api/activities"&&method==="POST"){
    ...
    const t=now(),a={id:id(),title:clean(b.title),description:clean(b.description)||null,team,assigned,priority:b.priority,location:clean(b.location)||null,due:new Date(due).toISOString()};
    await env.DB.prepare("INSERT INTO activities(id,title,description,team,assigned_to_id,priority,location,due_date,requires_observation,created_by_id,source,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(a.id,a.title,a.description,a.team,a.assigned,a.priority,a.location,a.due,bool(b.requiresObservation)?1:0,auth.user.id,auth.role.name==="admin"?"admin":"zeladoria",t,t).run();
    for(const list of Array.isArray(b?.checklists)?b.checklists:[]){const title=clean(list?.title)||"Checklist",items=Array.isArray(list?.items)?list.items.map(clean).filter(Boolean):[];if(!items.length)continue;const listId=id(),operations=[env.DB.prepare("INSERT INTO activity_checklists(id,activity_id,title,created_by_id,created_at) VALUES(?,?,?,?,?)").bind(listId,a.id,title,auth.user.id,t)];items.forEach((label,position)=>operations.push(env.DB.prepare("INSERT INTO activity_checklist_items(id,checklist_id,label,position) VALUES(?,?,?,?)").bind(id(),listId,label,position)));await env.DB.batch(operations);}
  ...
  if(activityMatch){
    ...
    if(!action&&method==="PATCH"){
      ...
      const fields=[["title","title"],["description","description"],["team","team"],["assignedToId","assigned_to_id"],["priority","priority"],["location","location"],["dueDate","due_date"]],sets=[],args=[],changed=[];
      for(const [input,col] of fields)if(Object.hasOwn(b,input)){sets.push(`${col}=?`);args.push(col==="assigned_to_id"?(clean(b[input])||null):col==="due_date"?new Date(clean(b[input])).toISOString():typeof b[input]==="string"?clean(b[input]):b[input]);changed.push(input)}
    ...
    if(action==="complete"&&method==="POST"){
      ...
      if(!result.meta?.changes)return json({error:"A atividade já foi alterada"},409);
      if(a.template_id){const template=await one(env.DB,"SELECT * FROM activity_templates WHERE id=?",a.template_id);if(template&&template.awaiting_activity_id===a.id){const following=nextRun(template,t);await env.DB.prepare("UPDATE activity_templates SET next_run_at=?,awaiting_activity_id=NULL,updated_at=? WHERE id=? AND awaiting_activity_id=?").bind(following,t,template.id,a.id).run();}}
